...
- ラグドールは移植していません。physics (Havok) に載せ替える際に自前実装を落としたため、
  敵の死亡は決定的な手続き倒れ込み (`src/ai/deathfall.js`) で代替しています。
  差し替え口 (begin/update/finish) は用意してあります。
- 兵士の 2 スケールディテールタイルとシルエットのリム減光が未移植です
  (`src/ai/textures.js` 冒頭に移植方針)。
