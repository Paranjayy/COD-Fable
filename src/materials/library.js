...
export const LIBRARY = {
  ...
  glass: {
    ...
    surface: 'glass',
    // pa = [拭き跡の濃さ, 埃の濃さ, -, -]
    bake: { size: 512, world: 2.0, relief: 0.004, tile: 4, seed: 109, pa: [0.22, 0.18, 0, 0] },
     ...
     * **意図的に法線が平坦**であることの宣言。
     *
     * tools/matbake.mjs のゲートは「法線がフラット = Sobel が効いていない」と
     * 判定するが、ガラスに限ってはそれが正しい状態。フロートガラスの実際の
     * うねりは mm オーダーで、法線マップとしてはほぼ無に等しい (見た目に効くのは
     * 反射側であって凹凸ではない)。
     *
     * ゲートを緩めるのではなくサーフェス側で例外を宣言するのは、残り 18 面に
     * 対する検査を厳格なまま保つため。例外は必ずここに列挙され、レビューできる。
     */
    expectFlat: true,
  },
};
