...
export const SHOTS = {
   ...
   * 薄暮のマズルフラッシュ。
   *
   * **昼の `muzzle` ショットではこのショットの目的を果たせない。** 銃口の輝度は
   * 実測で 216、すぐ隣の日向の背景が 200 前後あるので、フラッシュは写ってはいても
   * 形が読めない。屋外の日中に銃口炎がほとんど見えないのは物理的に正しい挙動なので、
   * ショット側を暗所に置く。
   *
   * 昼の `muzzle` は残してある。ビューモデルの姿勢と反動を明るい条件で見るショットと
   * しては引き続き機能するし、過去のベースラインとの比較を壊さないため。
   */
  muzzle_dusk: {
    pos: [6, 1.7, 10],
    look: [-2, 1.8, -2],
    ...
    time: 20.6,
    apply: (e, o) => e.ctx.peek('weapons')?.debugPose?.('fire', o),
    doc: '薄暮のマズルフラッシュ — 炎の形、ローブ、光の回り込み。昼の muzzle では読めない。',
  },
