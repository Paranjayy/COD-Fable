...
export class RenderSystem {
  ...
  _setupPipelines(ctx, q) {
    ...
    if (q.taa) {
      ...
      /**
       * **`clampHistory` は必ず有効にすること。既定は false。**
       *
       * 履歴ピクセルを周囲 3x3 の min/max にクランプする、いわゆる neighborhood
       * clamping。これが無い TAA は「履歴 92% + 現フレーム 8%」の単純な指数移動平均で
       * しかなく、**数フレームしか存在しない現象を完全に潰す**。
       *
       * 実測 (muzzle ショット, lockstep, 銃口 1000-1090 x 620-690 の最大輝度):
       *   taa なし        219-255 が毎フレーム、位置は銃口 (1044-1046, 652-659)
       *   taa あり (既定)  196-208、位置は矩形の角 = **フラッシュが 1 フレームも写らない**
       *
       * つまりマズルフラッシュ (寿命 40-62 ms = 2-3 フレーム) は TAA に完全に食われて
       * いた。撮影のタイミングの問題ではない — **実プレイでも銃口は光っていなかった**。
       * 発射 14 フレーム分を 1 フレームずつ撮って確認済みで、遅れて出るのでもない。
       */
